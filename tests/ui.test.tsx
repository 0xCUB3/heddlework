...
describeNative('WorkbenchApp', () => {
  it('renders and operates the T3-style native workbench shell', async () => {
    ...
    await Bun.sleep(80)
    root.renderer.flush()
    expect(await automation.getByTestId('copy-message').count()).toBeGreaterThanOrEqual(1)
    expect(await automation.getByTestId('fork-message').count()).toBeGreaterThanOrEqual(1)
    if (process.platform === 'darwin') {
