            ...
            {options.map((option) => {
              ...
              return (
                <div key={option.id} testId={`flow-priority-option-${option.id}`} tabIndex={0} style={{ position: 'relative', height: 34, width: '100%', minWidth: 0, display: 'flex', flexDirection: 'row', alignItems: 'center', gap: 8, paddingLeft: 8, paddingRight: 8, borderRadius: 6, backgroundColor: active ? colors.hover : colors.popover, pointerEvents: 'auto', cursor: 'pointer' }}>
                  ...
                  <text style={{ minWidth: 0, flexGrow: 1, color: active ? colors.text : colors.textMuted, fontSize: 10, fontWeight: 500, whiteSpace: 'nowrap', pointerEvents: 'none' }}>{option.label}</text>
...
export function FlowPriorityIcon({ priority, monochrome = false, testId = 'flow-priority-icon' }: { priority: ThreadPriority; monochrome?: boolean | undefined; testId?: string | undefined }) {
  ...
  // 6px stem + 1px gap + 2px dot = 9px, which centres on whole pixels only inside an odd box; 14px lands on a half pixel at 1x scale.
  if (priority === 1) return <div testId={testId} style={{ width: 15, height: 15, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 1, borderRadius: 4, backgroundColor: tone, pointerEvents: 'none' }}><div testId={`${testId}-urgent-stem`} style={{ width: 2, height: 6, borderRadius: 1, backgroundColor: colors.background, pointerEvents: 'none' }} /><div testId={`${testId}-urgent-dot`} style={{ width: 2, height: 2, borderRadius: 1, backgroundColor: colors.background, pointerEvents: 'none' }} /></div>
