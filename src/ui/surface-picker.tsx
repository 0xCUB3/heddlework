...
export function SurfacePickerPanel({ surfaces, fullscreen, fullscreenProgress, panelWidth, onToggleFullscreen, onSelect, onClose }: { surfaces: readonly SurfaceDescriptor[]; fullscreen: boolean; fullscreenProgress?: number | undefined; panelWidth?: number; onToggleFullscreen(): void; onSelect(surfaceId: string): void; onClose(): void }) {
  return (
...
export function SurfacePlaceholderPanel({ descriptor, fullscreen, fullscreenProgress, panelWidth, onToggleFullscreen, onNew, onClose }: { descriptor: SurfaceDescriptor; fullscreen: boolean; fullscreenProgress?: number | undefined; panelWidth?: number; onToggleFullscreen(): void; onNew(): void; onClose(): void }) {
  return (
...
function SurfaceCard({ surface, onClick }: { surface: SurfaceDescriptor; onClick(): void }) {
  return (
    <div testId={`surface-option-${surface.id}`} tabIndex={0} style={{ width: '48%', minWidth: 170, minHeight: 92, display: 'flex', flexDirection: 'column', gap: 8, padding: 13, borderRadius: 9, borderWidth: 1, borderColor: colors.borderStrong, backgroundColor: colors.card, cursor: 'pointer', hover: { backgroundColor: colors.hover } }} onClick={onClick} onKeyDown={(event) => { if (event.key === 'enter') onClick() }}>
