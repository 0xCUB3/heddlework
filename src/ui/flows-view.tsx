...
function FlowTabButton({ id, active, label, count, icon, onClick }: { id: FlowTab; active: boolean; label: string; count: number; icon: IconName; onClick(): void }) {
  return <div testId={`flows-tab-${id}`} tabIndex={0} style={{ position: 'relative', height: 36, display: 'flex', flexDirection: 'row', alignItems: 'center', gap: 6, paddingLeft: 10, paddingRight: 10, cursor: 'pointer', userSelect: 'none' }} onClick={onClick}><Icon name={icon} size={11} color={active ? colors.textMuted : colors.textFaint} /><text style={{ color: active ? colors.text : colors.textMuted, fontSize: 10, fontWeight: 500 }}>{label}</text><text style={{ color: colors.textFaint, fontSize: 9 }}>{count}</text>{active && <div style={{ position: 'absolute', left: 7, right: 7, bottom: 0, height: 2, borderRadius: 1, backgroundColor: colors.primary }} />}</div>
}
