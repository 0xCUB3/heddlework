...
On the development machine, the direct path measured 2.58 ms median / 3.51 ms p95 at 220×65 and 10.61 ms median / 15.75 ms p95 at 480×120, with three retained nodes and GPUI paint at 0.10 ms p90 for the near-4K grid. Before fused parsing, packed snapshots, and direct staging, the same 480×120 path took about 26 ms median. Treat timings as hardware-dependent; synchronized provenance, realistic byte ingress, direct-transport capability, workload shape, node count, and immutable row reuse are the structural guards.
