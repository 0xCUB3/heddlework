...
It reports incremental snapshots, a 5 MiB ANSI parse, packed-frame projection, unchanged-row identity reuse, and a synchronized 160×50 framebuffer through VT parsing, React, NAPI, GPUI layout, and paint. Every framebuffer cell uses an absolute cursor move, truecolor foreground/background, a Unicode block, and SGR reset—the control mix that exposed the Golden Star regression. On the development machine after the full test suite, this complete frame measured 4.47 ms median / 6.79 ms p95 with three retained nodes; native paint itself measured 0.11 ms p90, and a warmed 522 KiB frame split into 1 KiB PTY fragments coalesced in 0.25 ms. A captured 499 KiB Golden Star frame dropped from 34.54 ms to 3.17 ms median VT parse time. The live 220×65 Golden Star probe improved from 37 complete frames in 3.02 seconds (12.3 FPS) to 168 in 3.00 seconds (55.9 FPS), with native paint at 0.83 ms p90. Treat timings as hardware-dependent; the packed payload, workload shape, node count, and row reuse are the structural guards.
