...
The UI probe pre-encodes output outside the timed region, fragments it into 1 KiB PTY callbacks, and measures synchronized scanning, VT parsing, packed projection, direct NAPI staging, GPUI layout, and paint. Every framebuffer cell uses an absolute cursor move, truecolor foreground/background, a Unicode block, and SGR reset—the control mix that exposed the Golden Star regression. The high-resolution command covers 220×65, 320×90, and 480×120 rather than extrapolating from a small grid, and intentionally fails unless the patched native test renderer exposes the direct binary transport.

On the development machine, repeat runs of the byte-native direct path measured 1.17–1.18 ms median / 1.24–1.63 ms p95 at 220×65 and 4.03–4.64 ms median / 5.32–7.12 ms p95 at 480×120, with three retained nodes and GPUI paint at 0.04–0.09 ms p90 for the near-4K grid. The immediately preceding decoded-string path took 10.47 ms median, while the original pre-fusion path took about 26 ms median at 480×120. Treat timings as hardware-dependent; synchronized provenance, realistic byte ingress, direct-transport capability, workload shape, node count, and immutable row reuse are the structural guards.
