...
export function verifyCefArtifactInventory(root: string, expected: unknown): void {
  ...
  for (const path of expectedPaths) {
    const entry = expected[path]
    if (!validEntry(entry) || JSON.stringify(actual[path]) !== JSON.stringify(entry)) {
...
function validEntry(value: unknown): value is CefArtifactEntry {
  if (!isRecord(value)) return false
  if (value.type === 'directory') return validMode(value.mode) && Object.keys(value).length === 2
  if (value.type === 'symlink') return typeof value.target === 'string' && Object.keys(value).length === 2
